<template>
  <div>
    <div class='source'>
      <el-page-header content='添加用户' @back='back' />
    </div>
    <div class='source'>
      <el-row>
        <el-form ref='user' :model='user' label-width='150px' :inline='false' class='demo-form-inline'
          ...
          <el-form-item label='账号状态' prop='status'>
            <el-select v-model='user.status' placeholder='请选择'>
              <el-option
                v-for="item in [{'name':'激活','value':'0'},{'name':'冻结','value':'1'}]"
                :key='item.name'
                :label='item.name'
                :value='item.value'>
              </el-option>
...
import { checkRealName, checkUserName, createFosUser, getDepartmentByGroupIdAndPostName, getGroupAll } from '@/api/user'
