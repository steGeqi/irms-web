<template>
  <div style='height: auto'>
    ...
    <el-row :gutter='20' style='margin-bottom: 10px;'>
      <el-col :span='12'>
        <el-label>是否国产化：</el-label>
        <el-autocomplete
          popper-class='my-autocomplete'
          v-model='infoInput.isChinaLocalization'
          :fetch-suggestions="((queryString,cb)=>{querySearch(queryString,cb,'edition')})"
          placeholder='请输入是否国产化'
          clearable
          @select="handleSelect('isChinaLocalization',$event)"
        >
          <template slot-scope='{ item }'>
            <div class='name'>{{ item.isChinaLocalization }}</div>
          </template>
        </el-autocomplete>
      </el-col>
    </el-row>
